var App = function () {
	...
	var handleRaty = function () {
		$.fn.raty.defaults.path = base_url+'assets/js/jquery-raty/img';
		...
		$('#starHalf-demo').raty({
			path    : base_url+'assets/js/jquery-raty/img',
			half    : true,
	...
	$('.radio1').on('switch-change', function () {
		...
		//$('input[name="status"]').bootstrapSwitch('toggleState', true, true);
	}
	/*-----------------------------------------------------------------------------------*/
	/*	jQuery UI Sliders
	/*-----------------------------------------------------------------------------------*/
	...
	var handleDataTables = function () {
		$('#datatable1').dataTable({
                ...
                oTableTools: {
                    ...
                    sSwfPath: base_url+"assets/js/datatables/extras/TableTools/media/swf/copy_csv_xls_pdf.swf"
                }
			});
...
/*-----------------------------------------------------------------------------------*/
	...
	/*-----------------------------------------------------------------------------------*/
	var handleMultipleDataTables = function () {
	...
	var handleBackstretch = function () {
		 $.backstretch([
		  base_url+"assets/img/login/1.jpg"
		, base_url+"assets/img/login/2.jpg"
		, base_url+"assets/img/login/3.jpg"
		, base_url+"assets/img/login/4.jpg"
	...
	var handleChat = function (elem) {
		var append = function() {
			...
			var msg = '';
				...
				msg +=	'<img class="media-object" alt="Generic placeholder image" src="'+base_url+'assets/img/chat/headshot2.jpg">';
				msg +=  '</a>';
	...
	var handleTimeline = function () {
		createStoryJS({
			...
			width:		'100%',
			height:		'600',
			source:		base_url+'assets/js/timelinejs/example_json.json',
			embed_id:	'my-timeline',
			debug:		true,
			css:        base_url+'assets/js/timelinejs/css/timeline.css',     
            js:         base_url+'assets/js/timelinejs/js/timeline-min.js'    
		});
	}
	/*-----------------------------------------------------------------------------------*/
	/*	Handle Slidernav
	...
	var handleSliderNav = function () {
		...
		$('#address-book .slider-content ul li ul li a').click(function(e){
			...
			//Set the image
			$('#contact-card .headshot img').attr('src', base_url+'assets/img/addressbook/'+img_id+'.jpg');
	...
	var handleThemeSkins = function () {
        ...
        var setSkin = function (color) {
            $('#skin-switcher').attr("href", base_url+"assets/css/themes/" + color + ".css");
	...
	return {
        ...
        init: function () {
			...
			if (App.isPage("widgets_box")) {
				//handleUniform();// || App.isPage("sub_order")
				handleBoxSortable(); //Function to handle Box sortables
            }
			if (App.isPage("new_employee")) {
				//handleDatepicker();
				handleUniform();
			...
			if (App.isPage("button_icons") || App.isPage("sub_order")) {
			...
			if (App.isPage("manage_languages")) {
				handleTablecloth();	//Function to display tablecloth.js options
			}
			if (App.isPage("dashboard_!")     || App.isPage("customers_information")|| 
			    App.isPage("manage_email_templates")||App.isPage("sms_templates")  ||App.isPage("manage_sender")  ||
				App.isPage("subcategories")        || App.isPage("categories")    || 
				App.isPage("sizes")  || App.isPage("suppliers_information")      || 
				App.isPage("list_products")   || App.isPage("employees_information") || App.isPage("cloth_purchase")  	  ||
				App.isPage("list_purchases")      || App.isPage("list_cloth_products")    ||
				App.isPage("measurement") 	    || App.isPage("assigned_orders") 		 ||
				App.isPage("cloth_styles") 	    || App.isPage("taxes") 		 ||
				App.isPage("view_suborder")|| App.isPage("list_orders")	  ||
				App.isPage("cloth_types") || App.isPage("stitching") || App.isPage("pattern") ||	  
				App.isPage("style") || App.isPage("completed_orders") || App.isPage("discounts") ||App.isPage("bonus") ||
				App.isPage("order_payment") || App.isPage("view_invoice") ||
				App.isPage("search_orders") || App.isPage("track_suborder")|| 
				App.isPage("ready_on_time") || App.isPage("ready_after_time")|| 
				App.isPage("not_ready_till_time") || App.isPage("new_in_process")|| 
				App.isPage("today_new_order") || App.isPage("today_ready_order") || 
				App.isPage("date_today_not_ready") ||
				App.isPage("purchase_reports") || App.isPage("product_alert") || 
				App.isPage("track_orders") || App.isPage("search_orders") || App.isPage("orders") ||
				App.isPage("delivered_orders") || App.isPage("worker_report")||
                App.isPage("transaction_report") || App.isPage("track_worker_report") 
                || App.isPage("view_new_suborder")
                || App.isPage("employee_bonus") || App.isPage("logs_history")) {
			...
			/*if(App.isPage("dashboard"))
			{
				handleMultipleDataTables();	
			}*/
			
			if (App.isPage("jqgrid_plugin")) {
				handleJqgrid();	//Function to display jqGrid
			}
			if (App.isPage("forms") || App.isPage("new_customer")) {
			...
			if (App.isPage("new_product") || App.isPage("new_purchase")) {
				handleTypeahead();	//Function to display autocomplete
				handleAutosize();
				handleCountable();
			}
			if (App.isPage("email_templates")) {
			...
			if (App.isPage("login_bg") || App.isPage("login")) {
				handleUniform();	//Function to handle uniform inputs
			...
			if (App.isPage("user_profile")) {
				handleProfileSkillPie(); //Function to show skills in pie
					//Function to display sparklines
				handleUniform();	//Function to handle uniform inputs
        ...
        blockUI: function (el, loaderOnTop) {
            lastBlockedUI = el;
            jQuery(el).block({
                message: '<img src='+base_url+'assets/img/loaders/12.gif" align="absmiddle">',
                css: {
                    border: 'none',
                    padding: '2px',
