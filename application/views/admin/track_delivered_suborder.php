								...
								<div class="box border blue">
									...
									<div class="box-body">
										<table id="datatable2" cellpadding="0" cellspacing="0" border="0" class="datatable table table-striped table-bordered table-hover">
											...
											<a href="javascript:;" onclick="confirm_bootbox('<?=base_url().$this->session->userdata('roles')?>/track_order/inprocess_status/<?php echo $order['id'];?>');"><?php echo get_phrase('assigned');?></a>
                                            <?php }elseif($order['status']=='inprocess'){?>
                                            <a href="javascript:;" onclick="confirm_bootbox('<?=base_url().$this->session->userdata('roles')?>/track_order/completed_status/<?php echo $order['id'];?>');"> <?php echo get_phrase('proceed');?> <?php echo get_phrase('for');?> <?php echo get_phrase('stitch');?></a>
                                            <?php }elseif($order['status']=='completed'){?>
                                            <a href="javascript:;" onclick="confirm_bootbox('<?=base_url().$this->session->userdata('roles')?>/track_order/to_deliver/<?php echo $order['id'];?>');"> <?php echo get_phrase('ready');?> <?php echo get_phrase('from');?> <?php echo get_phrase('worker');?> </a>
											 <?php }elseif($order['status']=='to_deliver'){?>
                                            <a href="javascript:;" onclick="confirm_bootbox('<?=base_url().$this->session->userdata('roles')?>/track_order/delivered_status/<?php echo $order['id'];?>');"> <?php echo get_phrase('ready');?> <?php echo get_phrase('to');?> <?php echo get_phrase('deliver');?> </a>
											<?php }else{
												?>
