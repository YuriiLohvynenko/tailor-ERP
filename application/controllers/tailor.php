...
*/
	function __construct(){
		...
		if(!$this->session->userdata('user_id') || $this->session->userdata('roles') != 'tailor' ){
	...
	public function dashboard(){
		$page_data['page_name']  = 'dashboard';
        $page_data['page_title'] = get_phrase('dashboard');
		$page_data['workerOrders'] = $this->order_model->getWorkerOrdersById($this->session->userdata('user_id'));
		$page_data['completeOrders']  = $this->order_model->getWorkerCompletedOrdersById($this->session->userdata('user_id'));
		$page_data['todayReadyOrders'] = $this->order_model->getTodayReadyOrders($this->session->userdata('user_id'), 'worker');
		$page_data['todayNotReadyOrders'] = $this->order_model->getTodayNotReadyOrders($this->session->userdata('user_id'), 'worker');
		$page_data['delivers'] = $this->order_model->workerReportsbyStatus($this->session->userdata('user_id'), 'to_deliver','delivered');
	...
	public function completed_orders()
	{
		...
		$page_data['suborders']  = $this->order_model->getWorkerCompletedOrdersById($this->session->userdata('user_id'));
        $this->load->view('index', $page_data);
	}
	
	public function track_orders()
	{
		$page_data['page_name']  = 'track_orders';
        $page_data['page_title'] = get_phrase('track')." ".get_phrase('orders');
		$page_data['suborders']  = $this->order_model->getWorkerOrdersById($this->session->userdata('user_id'));
		//echo "<pre>";
		//print_r($page_data['suborders']);
        $this->load->view('index', $page_data);
	}
	
	public function track_order($param1='', $param2='', $param3='')
	{
	...
	public function bonus($param1 = '')
	{
		if ($param1 == 'search') {
			...
			{
				$data['employee']  = $this->session->userdata('user_id');
